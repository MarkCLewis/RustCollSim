...
use crate::design::system::Particle;
...
impl Grid {
    ...
    fn push(&mut self, particle: usize, x: f64, y: f64, r: f64) {
    ...
    fn get_neighbors(&self, x: f64, y: f64) -> Vec<usize> {
        let cell_x = (x / self.cell_size).floor() as i32;
        let cell_y = (y / self.cell_size).floor() as i32;
    ...
    fn collides_with(&self, p: &Particle, population: &Vec<Particle>) -> bool {
        self.get_neighbors(p.x.x(), p.x.y())
            .into_iter()
            .any(|n| {
              population[n].is_colliding(&p)
            })
    }
    ...
    fn try_push(
        ...
        if self.collides_with(&particle, population) {
            ...
            self.push(particle_idx, particle.x.x(), particle.x.y(), particle.r);
            Ok(())
        }
    }
}
    ...
    F: FnMut() -> Particle,
{
    ...
    for i in 0..count {
        loop {
            let p = particle_gen();
            if grid.try_push(&p, i, &population).is_ok() {
                population.push(p);
                break;
            }
        }
